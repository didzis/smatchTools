...
class Bottle(object):
    ...
    def _handle(self, environ):
        ...
        try:
            ...
            try:
                yield from self.trigger_hook('before_request', request, response)
                ...
                yield from self.trigger_hook('after_request', request, response)
...
class AioHTTPServer(ServerAdapter):
    ...
    def run(self, app): # pragma: no cover
        ...
        server = loop.run_until_complete(loop.create_server(
                lambda: WSGIServerHttpProtocol(app, readpayload=True, keep_alive=1500), self.host, self.port))
